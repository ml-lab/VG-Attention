...
cmd:text()
cmd:text('Options')
cmd:option('-input_json','../data/VG_data_prepro_img_feat.json','path to the json file containing vocab and answers')
...
cmd:option('-out_name_train', '../data/VG_data_img_residule_train_final.h5', 'output name train')
cmd:option('-out_name_val', '../data/VG_data_img_residule_val_final.h5', 'output name val')
cmd:option('-out_name_test', '../data/VG_data_img_residule_test_final.h5', 'output name test')
...
for i,imname in pairs(json_file['unique_img_val']) do
    table.insert(img_list_val, image_root .. imname)
end
...
local feat_train=torch.FloatTensor(sz,2048) --ndims)
...
for i=1,sz,batch_size do
    ...
    feat_train[{{i,r},{}}]=output:contiguous():float()
    collectgarbage()
end
...
local ndims=4096
local batch_size = opt.batch_size
local sz=#img_list_val
local feat_val=torch.FloatTensor(sz,2048) --ndims)
print(string.format('processing %d images...',sz))
for i=1,sz,batch_size do
    xlua.progress(i, sz)
    r=math.min(sz,i+batch_size-1)
    local ims=torch.CudaTensor(r-i+1,3,224,224)
    for j=1,r-i+1 do
        ims[j]=loadim(img_list_val[i+j-1]):cuda()
    end
    local output = model:forward(ims)
    feat_val[{{i,r},{}}]=output:contiguous():float()
    collectgarbage()
end

local val_h5_file = hdf5.open(opt.out_name_val, 'w')
val_h5_file:write('/images_val', feat_val)
val_h5_file:close()

local ndims=4096
local batch_size = opt.batch_size
local sz=#img_list_test
local feat_test=torch.FloatTensor(sz,2048)
print(string.format('processing %d images...',sz))
for i=1,sz,batch_size do
    xlua.progress(i, sz)
    r=math.min(sz,i+batch_size-1)
    ...
    local output = model:forward(ims)
    feat_test[{{i,r},{}}]=output:contiguous():float()
    collectgarbage()
end
